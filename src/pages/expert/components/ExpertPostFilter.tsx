...
import { useControlPostFilter } from '@expert/features/hooks/useControlPostFilter.ts';

const ExpertPostFilter = () => {
  const { isOn, filter, filterItems, changeFilter, handleToggle } = useControlPostFilter();
      ...
      {isOn && (
        <FilterItemsContainer>
          {filterItems.map((item, index) => (
            <p key={index} onClick={() => changeFilter(index)}>
              {item}
            </p>
          ))}
        </FilterItemsContainer>
...
export default ExpertPostFilter;
